...
def draw_init():
    '''creates a surface for the board'''
    board_surface.fill(board_color)

    for x in range(1, game_size):
        for y in range(1, game_size):
            pygame.draw.line(board_surface,BLACK,(x*unit_size,unit_size),(x*unit_size,unit_size*(game_size-1)),3)
            pygame.draw.line(board_surface,BLACK,(unit_size,y*unit_size),(unit_size*(game_size-1),y*unit_size),3)
...
def mouse_clicked(event):
    '''Checks mouse coordinates, prints to terminal'''
    print(pos2grid(event.pos))
    #dd_stone(white_dict,"white",pos2grid(event.pos))


def pos2grid(pos):
    '''Converts pixel location to board coordinates'''
...
def grid2pos(grid):
    '''converts grid position to pixel location'''
...
def make_board_tuple():
    ...
    '''
    return [(j, i) for i in range(game_size-1) for j in range(game_size-1)]
